...
export function PingAgentButton() {
  const [isPending, startTransition] = useTransition();
  const router = useRouter();

  const ping = () => {
    const title = window.prompt("What should sysadmin do?");
    if (!title?.trim()) return;
    startTransition(async () => {
      const res = await fetch("/api/tasks", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ title, agent: "sysadmin" }),
      });
      if (!res.ok) {
        const { error } = (await res.json().catch(() => ({ error: "write failed" }))) as {
          error?: string;
        };
        alert(`Failed to ping sysadmin: ${error ?? "unknown error"}`);
        return;
      }
      router.refresh();
    });
