...
export function SessionsList({ sessions }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      {filtered.length === 0 ? (
        <div className="text-[14px] text-muted py-8 text-center border border-dashed border-border rounded-md">
              ...
              <li key={`${s.source}/${filename}`}>
                ...
                >
                  <div className="flex items-baseline gap-2 flex-wrap">
                    <span
                      className={`text-[12px] px-1.5 py-0.5 rounded shrink-0 ${SOURCE_STYLE[s.source]}`}
                    >
                  ...
                  {s.frontmatter.outcome && (
                    <p className="mt-1 text-[14px] text-muted line-clamp-2">
                      {s.frontmatter.outcome}
                    </p>
                  )}
                </Link>
